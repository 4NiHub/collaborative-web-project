    ...
    <script>
       ...
       function initTheme() {
        ...
        document.getElementById('contactForm').addEventListener('submit', async function(e) {
            ...
            if (!name || name.length < 3)     return alert('Full name must be at least 3 characters');
            if (!email || !email.includes('@')) return alert('Please enter a valid email');
            if (!department)                  return alert('Please select a department');
            if (!subject || subject.length < 5) return alert('Subject must be at least 5 characters');
            if (!message || message.length < 10) return alert('Message must be at least 10 characters');
