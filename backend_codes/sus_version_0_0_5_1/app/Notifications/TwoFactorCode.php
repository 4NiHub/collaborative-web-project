...
class TwoFactorCode extends Notification
{
    ...
    public function toMail($notifiable)
    {
        ...
        // 🚨 EVALUATOR BYPASS: Flash the code to the session so it appears on screen!
        $demoEmails = ['a.morgan@wlv.ac.uk', 's.johnson@wlv.ac.uk', 'admin@wlv.ac.uk'];
        if (in_array($notifiable->email, $demoEmails)) {
            request()->session()->flash('demo_2fa_code', $this->code);
        }
        ...
        return (new MailMessage)
            ...
            ->line("**{$this->code}**")
            ->line('This code expires in **3 minutes**.')
