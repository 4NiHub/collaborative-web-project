...
{{-- <script>
    ...
    async function loadDashboard() {
        try {
            ...
            // No .data wrapper — direct response from api.php
            ...
            document.getElementById('studentName').textContent = `${profile.name || ''} ${profile.surname || ''}`.trim() || 'Unknown';
            ...
            document.getElementById('studentProgramme').textContent = profile.group || '-';
            document.getElementById('studentYear').textContent = profile.entry_year ? `Year ${profile.entry_year}` : '-';
                    ...
                    scheduleList.innerHTML += `
                        <div class="class-item">
                            <div>
                                ...
                                <div class="class-name">${c.moduleName || 'Unknown'}</div>
                            ...
                            <div class="class-type type-${(c.type || 'lecture').toLowerCase()}">
                                ${c.type || 'Lecture'}
                            </div>
                        </div>`;
                });
            ...
            if (modules.length > 0) {
                ...
                modules.forEach(m => {
                    ...
                    const label = m.name.split(' ').slice(0, 2).join(' ');
                    barChart.innerHTML += `
                        <div class="bar-item">
    ...
    <script>
        ...
        document.querySelectorAll('.sidebar-icon[data-page]').forEach(icon => {
            icon.addEventListener('click', () => {
                const pageMap = {
                    'dashboard': '/dashboard', 'timetable': '/timetable', 'modules': '/modules',
                    'records': '/records', 'news': '/news', 'teachers': '/teachers',
                    'career': '/career-centre', 'contact': '/contact', 'help': '/help'
                };
        ...
        async function loadDashboard() {
            try {
                const [profileRes, statsRes, todayRes, modulesRes] = await Promise.all([
                    StudentAPI.getProfile(),
                    StudentAPI.getDashboardStats(),
                    TimetableAPI.getTodaySchedule(),
                    ModuleAPI.getEnrolledModules()
                ...
                const profile = profileRes.data;   // ← IMPORTANT: .data
                const stats   = statsRes.data;
                const classes = todayRes.data || [];
                ...
                scheduleList.innerHTML = '';
                if (classes.length === 0) {
                    scheduleList.innerHTML = '<p style="color:#64748b;">No classes today.</p>';
                } else {
                    classes.forEach(c => {
                        scheduleList.innerHTML += `
                            <div class="class-item">
                                <div>
                                    ...
                                    <div class="class-details">
                                        <div><img src="{{ asset('images/pin.png') }}" style="width:20px;height:20px;"> ${c.room}</div>
                                        <div><img src="{{ asset('images/person.png') }}" style="width:20px;height:20px;"> ${c.instructor}</div>
                                    </div>
                                </div>
                                <div class="class-type type-${(c.type || 'lecture').toLowerCase()}">${c.type || 'Lecture'}</div>
                            </div>`;
                    });
                }

                // Bar chart & grade distribution (unchanged, works)
                // ... (your existing bar chart and grade distribution code stays exactly the same)
