...
<body>
    ...
    <script>
        ...
        async function loadTimetable() {
            try {
                var res  = await TimetableAPI.getWeeklyTimetable();
                ...
                var groupsRes   = await TimetableAPI.getGroups();
                var teachersRes = await TeacherAPI.getTeachers();
                ...
                console.error('[Timetable] Load failed:', err.message);
                document.getElementById('calendarGrid').innerHTML =
                    '<p style="padding:40px;color:#dc2626;grid-column:1/-1;text-align:center;">Could not load timetable. Please refresh.</p>';
            }
        }
        ...
        function renderCalendar() {
            ...
            for (var day = 1; day <= daysInMonth; day++) {
                ...
                events.forEach(function(event) {
                    var instructor = (currentView === 'teacher') ? '' : (event.instructor || '');
                    var group      = (currentView === 'group')   ? '' : (event.group || '');
                    var typeClass  = 'event-' + (event.type || 'lecture').toLowerCase();
                    eventsHtml +=
